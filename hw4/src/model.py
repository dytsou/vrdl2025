...
def to_3d(x):
    """
    Convert a 4D tensor to a 2D tensor.
    """
...
def to_4d(x, h, w):
    """
    Convert a 2D tensor to a 4D tensor with specified height and width.
    """
...
class BiasFreeLayerNorm(nn.Module):
    """
    BiasFreeLayerNorm for image restoration.
    """
    ...
    def forward(self, x):
        """
        Forward pass for BiasFreeLayerNorm.
        """
...
class WithBiasLayerNorm(nn.Module):
    """
    WithBiasLayerNorm for image restoration.
    """
    ...
    def forward(self, x):
        """
        Forward pass for WithBiasLayerNorm.
        """
...
class LayerNorm(nn.Module):
    """
    LayerNorm for image restoration.
    """
...
class FeedForward(nn.Module):
    """
    Gated-Dconv Feed-Forward Network (GDFN) for image restoration.
    """
...
class Attention(nn.Module):
    """
    Multi-DConv Head Transposed Self-Attention (MDTA) for image restoration.
    """
...
class FastHOGAwareAttention(nn.Module):
    """
    FastHOGAwareAttention for image restoration.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for FastHOGAwareAttention.
        """
        _, _, h, w = x.shape
        ...
        # 3) map angles from [-π, π] to [0, π]
        ang = ang % torch.pi

        # 4) compute soft assignment to orientation bins
        #    for each bin center c_k, weight = max(0, 1 - |ang - c_k|/(π/orientations))
...
class Resblock(nn.Module):
    """
    Residual block for image restoration.
    """
    ...
    def __init__(self, dim):
        ...
        self.body = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=3, stride=1,
...
class Downsample(nn.Module):
    """
    Downsample for image restoration.
    """

    def __init__(self, n_feat):
        super(Downsample, self).__init__()

        self.body = nn.Sequential(nn.Conv2d(n_feat, n_feat//2, kernel_size=3,
...
class Upsample(nn.Module):
    """
    Upsample for image restoration.
    """
...
class TransformerBlock(nn.Module):
    """
    TransformerBlock for image restoration.
    """
...
class OverlapPatchEmbed(nn.Module):
    """
    OverlapPatchEmbed for image restoration.
    """
...
class PromptGenBlock(nn.Module):
    """
    PromptGenBlock for image restoration.
    """

    def __init__(self,
                 prompt_dim=128,
                 prompt_len=PROMPT_CONFIG['prompt_len'],
                 prompt_size=96,
                 lin_dim=192):
    ...
    def forward(self, x):
        """
        Forward pass for PromptGenBlock.
        """
        b, _, h, w = x.shape
        emb = x.mean(dim=(-2, -1))
...
class PromptIR(nn.Module):
    ...
    def __init__(self,
        ...
        # Set default heads if not provided
        if heads is None:
        ...
        if self.decoder:
            ...
            self.noise_level2 = TransformerBlock(
                dim=int(dim*2**2) + prompt2_ch,
                num_heads=heads[1],
                ffn_expansion_factor=ffn_expansion_factor,
                bias=bias,
                LayerNorm_type=LayerNorm_type)
            self.reduce_noise_level3 = nn.Conv2d(
                int(dim*2**3) + prompt3_ch, int(dim*2**2),
                kernel_size=1,
                bias=bias)
