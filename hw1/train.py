...
CONFIG = {
    ...
    # Further increased reduction ratio for ResNet50
    "CBAM_REDUCTION_RATIO": 128,
...
class ImageDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None, is_test=False):
        ...
        if not is_test:
            ...
            self.class_to_idx = {
                cls_name: int(cls_name) for cls_name in self.classes
            }
            self.samples = []
...
class SimplifiedCBAM(nn.Module):
    ...
    def __init__(self, in_channels, reduction_ratio=64, kernel_size=3):
        super().__init__()
        self.channel_attention = SimplifiedChannelAttention(
...
class ImageClassifier(nn.Module):
    """
    Image classification model using ResNet50 with
    optional simplified CBAM.
    """
    ...
    def __init__(
        ...
        if CONFIG["USE_CBAM"]:
            if CONFIG["USE_SIMPLIFIED_CBAM"]:
                ...
                self.cbam1 = SimplifiedCBAM(
                    256,
                    reduction_ratio=CONFIG["CBAM_REDUCTION_RATIO"]
                )
                self.cbam2 = SimplifiedCBAM(
                    512,
                    reduction_ratio=CONFIG["CBAM_REDUCTION_RATIO"]
                )
                self.cbam3 = SimplifiedCBAM(
                    1024,
                    reduction_ratio=CONFIG["CBAM_REDUCTION_RATIO"]
                )
                self.cbam4 = SimplifiedCBAM(
                    2048,
                    reduction_ratio=CONFIG["CBAM_REDUCTION_RATIO"]
                )
            else:
