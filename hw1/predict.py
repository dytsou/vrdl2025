...
def predict():
    ...
    with torch.no_grad():
        for inputs, batch_filenames in test_loader:
            ...
            for filename, pred in zip(batch_filenames, predicted.cpu().numpy()):
                predictions_dict[os.path.splitext(filename)[0]] = int(pred)  # Convert to int
    ...
    if missing_labels > 0:
        ...
        original_df['pred_label'] = original_df['pred_label'].fillna(0).astype(int)
    else:
    ...
    print(f"Label distribution:\n{original_df['pred_label'].value_counts().sort_index()}")
    # Create solution.zip
